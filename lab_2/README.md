...
## Теория
Нейросети (и вообще любые численные модели) не умеют работать со строками:
Gender = "Male"
...
Female → 0
Male   → 1
One-Hot Encoding (для многокатегориальных)
Кодирование категориальных признаков — это не “улучшение данных”,
а необходимое условие, чтобы модель вообще могла работать.
MTRANS = ["Public", "Walking", "Automobile"]
Преобразуется в:
MTRANS_Public       = 1
MTRANS_Walking      = 0
MTRANS_Automobile   = 0
То есть параметров становится больше

Отличие от 1 лабы:
Тип данных:
смешанные:
числовые (Age, Height, Weight)
категориальные (Gender, MTRANS, CAEC)
Следствие:
✅ нужен этап кодирования
иначе нейросеть просто не запустится
Тип задачи:
классификация, а не регрессия
выход = класс (7 уровней ожирения)

Критерий	Лаба 1	Лаба 2
Тип задачи	Регрессия	Классификация
Целевая переменная	Число	Класс
Категориальные признаки	❌ Нет	✅ Есть
Кодирование категорий	❌ Не нужно	✅ Обязательно
Масштабирование	StandardScaler	MinMaxScaler
Метрики	RMSE, MAE, R²	Accuracy, Balanced Acc, F1
Выход модели	Dense(1)	Dense(num_classes, softmax)
