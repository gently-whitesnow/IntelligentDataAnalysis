...
## Теория
Регрессионный анализ — это раздел машинного обучения/статистики, где мы строим модель, которая предсказывает числовое значение (не класс), опираясь на входные признаки.
Полносвязная нейросеть — это нейросеть, где в каждом Dense-слое каждый нейрон соединён со всеми нейронами предыдущего слоя.
Типичная регрессионная MLP (многослойный перцептрон):
X - Dense(\text{ReLU}) - Dense(\text{ReLU}) - Dense(1)

ReLU - rectified linear unit, функция активации, которая возвращает 0, если вход меньше 0, и x, если вход больше 0.
Как она делает регрессию - Обучение идёт через минимизацию ошибки (например, MSE) градиентным спуском.
RMSE (корень из MSE):

Одномерной сверточной нейронной сети - это нейросеть, которая применяет операцию свёртки по одной оси (обычно по времени или по порядку элементов) и умеет автоматически выделять локальные шаблоны в последовательностях: куски сигналов, “мотивы” во временных рядах, n-граммоподобные паттерны в тексте и т.п.

Строить одномерные распределения (гистограмма + boxplot) для каждого признака нужно не “для красоты”, а чтобы быстро увидеть свойства данных, которые потом напрямую влияют на качество регрессии и корректность предобработки

MAE — Mean Absolute Error (средняя абсолютная ошибка) «На сколько я ошибусь в MPa?»
R² — коэффициент детерминации показывает, какую долю вариативности данных модель смогла объяснить. «Насколько хорошо я вообще понял структуру данных?» (0.89 - на 89 % модель поняла структуру данных)
RMSE сильно штрафует большие ошибки
